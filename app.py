...
def make_gesture_control_dict():
    return {
        'Open': {
            'Right': do_volume_increase,
            'Left': do_volume_decrease
        }
    }
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if is_reboot or is_shutdown:
                    spend_time = time.time() - prev_time
                    if math.floor(spend_time) == 1:
                        n = n+1
                        prev_time = time.time()
                        beep()
                        print('Beep')
                    if n == 5:
                        do_reboot() if is_reboot else do_shutdown()
                else:
                    n = 0
                    prev_time = time.time()
...
def draw_info_text(image, brect, handedness, hand_sign_text, finger_gesture_text, gesture_control_dict):
    ...
    if hand_sign_text != "":
        if gesture_control_dict.get(hand_sign_text) and gesture_control_dict[hand_sign_text][info_text]:
            gesture_control_dict[hand_sign_text][info_text]()
